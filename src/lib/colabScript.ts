export const colabScriptContent = `# --- BLOCO CONSOLIDADO E CORRIGIDO PARA O COLAB ---
import os
import subprocess
import shutil
import asyncio
...
subprocess.run("sed -i 's/int(np.round(args.fps))/int(np.round(args.fps or 60))/g' inference_video.py", shell=True)
...
subprocess.run("pip install fastapi python-multipart uvicorn pyngrok nest-asyncio scikit-video numpy==1.26.4 tqdm psutil requests -q", shell=True)
patch_script = """
import os
import skvideo
